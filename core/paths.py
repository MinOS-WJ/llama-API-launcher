...
def _normalize_file_filters(file_filter):
    """Normalize comma-separated extensions from the UI into lowercase suffixes."""
    if not file_filter:
        return []
    result = []
    for raw in str(file_filter).split(","):
...
def _is_browsable_file(name, file_filter=None):
    """判断文件是否在目录浏览器中显示（模型/配置/可执行文件）。"""
    lower = name.lower()
    filters = _normalize_file_filters(file_filter)
    if filters:
        if "*" in filters:
            return True
        return any(lower.endswith(ext) for ext in filters)
    if lower.endswith(BROWSE_FILE_EXTS):
        return True
    return any(lower.startswith(p) for p in BROWSE_FILE_PREFIXES)


def browse_directory(path, file_filter=None):
    """浏览服务端目录，返回 {path, parent, entries, error}。
    path 为空时返回根列表；entries 中目录在前、文件在后，按名排序。
    file_filter 可选，逗号分隔扩展名；为空时保持旧版默认过滤。
    永不抛异常，错误写入 error 字段。
    """
    if not path:
        roots = list_roots()
    ...
    try:
        for entry in resolved.iterdir():
            try:
                ...
                elif entry.is_file() and _is_browsable_file(entry.name, file_filter):
                    try:
