...
2. **`/api/*` 受保护端点** — **本机与远程均需凭证**（密码设置后所有人都须登录）。接受两类凭证：`sk-` admin Key / `sess-` session。失败返回字符串错误格式（403），session 失效附 `auth_expired: true`。
...
- 208 项测试（42 项跳过 = httpx 未安装时的 TestClient 条件跳过）
