...
- 两类凭证分流：`sk-` admin Key / `sess-` session
...
3. 设置凭证后，所有请求危险接口（`/api/start`、`/api/stop`、`/api/update`、`/api/shutdown`、`/api/config` POST、方案编辑、回滚等）——**无论本机还是远程**——都须携带 `Authorization: Bearer <session_or_key>` 头
...
> Key 明文格式为 `sk-<32位随机>`（与 OpenAI 官方 Key 格式一致），服务端只存 SHA-256 哈希与前缀，无法反查明文。停用 / 回收某 Key 后该 Key 立即失效，不影响其他调用方。
>
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/config` | GET | 获取当前配置（含 llama.cpp 检测状态、github_token_set、remote_access） |
| `/api/config` | POST 🔒 | 更新配置（llamacpp_dir / model_dir / host / port / allowed_origins / github_token） |
...
- 三类路径分流：
  ...
  - **`/api/*` 受保护管理端点**：**本机与远程均需凭证**（密码设置后所有人都须登录），接受两类凭证：
...
项目附带 `unittest` 测试套件（`tests/` 目录），共 208 项测试覆盖纯逻辑函数与 API 层：
...
测试不依赖真实 llama-server、真实模型文件或真实 GitHub 网络。`test_api_client.py` 用 `unittest.mock` mock 掉 `urllib.request.urlopen`；`test_v1_api.py` 用 `ThreadingHTTPServer` 起 mock llama-server + `TestClient` 验证 `/v1/*` 反向代理与认证中间件（需 `httpx`，仅测试依赖）；`test_admin_auth.py` 用 `TestClient` 验证密码哈希、session 管理、登录限流与 `/api/auth/*` 端点（52 项，需 `httpx`）。
