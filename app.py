...
class LlamaLauncherApp:
    ...
    def _build_ui(self):
        ...
        ttk.Label(nf, text="监听地址：").pack(side="left", padx=(0, 4))
        self.host_var = tk.StringVar(value=str(self.config.get("host", "127.0.0.1")))
        ttk.Entry(nf, textvariable=self.host_var, width=20).pack(side="left", padx=(0, 12))
        ttk.Label(nf, text="端口：").pack(side="left", padx=(0, 4))
        self.port_var = tk.StringVar(value=str(self.config.get("port", 12345)))
        ttk.Entry(nf, textvariable=self.port_var, width=8).pack(side="left")
    ...
    def _start(self):
        ...
        try:
            ...
            return
        host_val = self.host_var.get().strip()
